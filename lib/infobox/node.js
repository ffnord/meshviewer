define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showSite(d, config) {
    ...
    var label = entry.name ? entry.name : site
    if(entry.link)
      return function(el) {
        var a = document.createElement("a")
        a.innerHTML = label
  ...
  function showRootfsUsage(d) {
    if (!("rootfs_usage" in d.statistics))
      return undefined

    return function (el) {
      el.appendChild(showBar("rootfs_usage", d.statistics.rootfs_usage))
    }
  }
  ...
  return function(config, el, router, d) {
    ...
    attributeEntry(attributes, "Speicherplatz", showRootfsUsage(d))
